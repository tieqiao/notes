...
def fun_keypoints():
    #filename="/nfs/hpc/share/wangtie/2022/2/21/2D/data_2d_DevEv_S_12_01_MobileInfants_trim.npz"
    filename="/nfs/hpc/share/wangtie/2022/2/21/2D/data_2d_DevEv_S_12_01_BottomLeft_trim.npz"
    data=np.load(filename, allow_pickle=True)
    print(data['metadata'])
    keypoints=data['data'].item()['keypoints']
    print(len(keypoints))
    viewcnt=np.zeros(4)
    for i in range(len(keypoints)):
        ...
        for j in range(4):
            pts1=keypoints[i][j]
            # print(len(pts1))
            if len(pts1)==2:viewcnt[j]+=1
        continue

        view0=np.array(pts1[0][:,:2])
        view0=np.mean(view0,axis=0)
        print(view0)
            # return 
    print(viewcnt[1])
    flag=0
        ...
        if flag==0:
            ...
            for j in range(len(pts1)):
                ...
                for k in range(len(pts1)):
                    dist=np.linalg.norm(view0-kps[k])
                    if dist<20:
                        cnt[k]+=1
    if cnt[1]>cnt[0]:
        kps[0]=kps[1]
    ...
    for i in range(len(keypoints)):
        ...
        if len(pts1)==1:
            view0=np.array(pts1[0][:,:2])
            view0=np.mean(view0,axis=0)
            dist=np.linalg.norm(view0-kps[0])
            if dist<20:
                keypoints[i][1]=[]
        elif len(pts1)>1:
            mdist=99999999
...
def fun_keypoints3(keypoints,zidx):
    viewcnt=0
    for i in range(len(keypoints)):    
        pts1=keypoints[i][zidx]
        if len(pts1)==2:viewcnt+=1
    
    if viewcnt<100:return
...
def fun_keypoints2():
    ...
    print(len(keypoints))
    # for j in range(4):
    #     fun_keypoints3(keypoints,j)
    #fun_keypoints3(keypoints,2)
    viewcnt1=np.zeros(4)
    viewcnt2=np.zeros(4)
    for i in range(len(keypoints)):
        #print(i)
        for j in range(4):
            pts1=keypoints[i][j]
            # print(len(pts1))
            if len(pts1)==2:viewcnt2[j]+=1
            if len(pts1)==1:viewcnt1[j]+=1
            # return 
    for j in range(4):
        print(viewcnt1[j],viewcnt2[j])
    ...
    data['keypoints']=keypoints
        # print(lst_P[i].shape)
        # break
    #np.save('data_2d_DevEv_S_12_01_BottomLeft_trim1', data)
    np.save('data_2d_DevEv_S_12_01_MobileInfants_trim1', data)
    return keypoints
...
def pair():
    ...
    for i in range(len(keypoints)):
        ...
        fview1=0
        fview2=0
        if isinstance(view1, np.ndarray) and isinstance(view3, np.ndarray):
            flag1=1
            flag2=3
            fview1=view1
            fview2=view3
    ...
    for i in range(len(lst_idx)):
        ...
        # print(lst_P[i].shape)
        # break
    np.save('data_3d_DevEv_S_12_01_MobileInfants_trim.npy', data)
pair()
